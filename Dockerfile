...
# Build x86_64
RUN make -C build-x86_64 ARCH=x86_64 VENDOR_CERT_FILE=../waxar.der DISABLE_REMOVABLE_LOAD_OPTIONS=y TOPDIR=.. -f ../Makefile

# Build aarch64
RUN make -C build-aarch64 ARCH=aarch64 CROSS_COMPILE=aarch64-linux-gnu- VENDOR_CERT_FILE=../waxar.der DISABLE_REMOVABLE_LOAD_OPTIONS=y TOPDIR=.. -f ../Makefile
